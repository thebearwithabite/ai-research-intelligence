...
def extract_substack_content(newsletter_url: str, max_posts: int = 5) -> List[Dict]:
    ...
    # Enforce upper bound on max_posts locally as well
            ...
            if response.status_code == 200:
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    ...
                    if content_length > MAX_RESPONSE_SIZE_BYTES:
                        print(f"Truncating response from {post_url} (exceeded {MAX_RESPONSE_SIZE_BYTES} bytes)")
                        break
...
def handler(event):
    ...
    # Validate and limit newsletters (DoS protection)
    if not isinstance(newsletters, list):
        newsletters = [str(newsletters)]

    if len(newsletters) > MAX_NEWSLETTERS:
        print(f"Warning: Limiting newsletters from {len(newsletters)} to {MAX_NEWSLETTERS}")
        newsletters = newsletters[:MAX_NEWSLETTERS]

    posts_per_newsletter = job_input.get('posts_per_newsletter', 3)

    # Validate and limit posts per newsletter
    try:
        posts_per_newsletter = int(posts_per_newsletter)
    except (ValueError, TypeError):
        posts_per_newsletter = 3

    if posts_per_newsletter > MAX_POSTS_PER_NEWSLETTER:
        print(f"Warning: Limiting posts per newsletter from {posts_per_newsletter} to {MAX_POSTS_PER_NEWSLETTER}")
