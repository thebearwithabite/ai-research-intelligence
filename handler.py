...
def extract_substack_content(newsletter_url: str, max_posts: int = 5) -> List[Dict]:
    ...
    try:
        ...
        # Fetch feed content safely preventing SSRF via redirects
        try:
            response = safe_requests_get(rss_url, timeout=10)
            if response.status_code != 200:
                print(f"Failed to fetch RSS: {rss_url} Status: {response.status_code}")
                return posts
            feed_content = response.content
        except ValueError as e:
            print(f"Skipping unsafe/failed RSS URL {rss_url}: {e}")
            return posts
...
def scrape_post_content(post_url: str) -> str:
    ...
    try:
        ...
        # safe_requests_get handles redirect validation to prevent SSRF
