...
class TestHandlerDoS(unittest.TestCase):
    ...
    def test_handler_newsletters_limit(self, mock_analyze, mock_extract):
        ...
        # Expect success (truncation behavior)
        self.assertIsInstance(result, dict)
        self.assertNotIn("error", result)

        # Verify processing happened for truncated list
    ...
    def test_handler_posts_limit(self, mock_analyze, mock_extract):
        ...
        # Expect success (capping behavior)
        self.assertIsInstance(result, dict)
        self.assertNotIn("error", result)

        # Verify processing happened with capped limit
        mock_extract.assert_called_once_with('http://example.com/1', MAX_POSTS_PER_NEWSLETTER)
